...
class ServerPinger:
    ...
    def read_status(self):
        ...
        try:
            ...
            raise IOError("Received invalid status response: {}".format(e))
    ...
    def test_ping(self):
        ...
        if received_token != self.ping_token:
            raise IOError(
                "Received mangled ping response packet "
                "(expected token {ping_token}, received {received_token})".format(
                    ping_token=self.ping_token,
...
class PingResponse:
        ...
        class Player:
            def __init__(self, raw):
                if type(raw) is not dict:
                    raise ValueError("Invalid player object (expected dict, found {})".format(type(raw)))

                if "name" not in raw:
                    raise ValueError("Invalid player object (no 'name' value)")
                if not isinstance(raw["name"], str):
                    raise ValueError(
                        "Invalid player object (expected 'name' to be str, was {})".format(type(raw["name"])))
                self.name = raw["name"]

                if "id" not in raw:
                    raise ValueError("Invalid player object (no 'id' value)")
                if not isinstance(raw["id"], str):
                    raise ValueError("Invalid player object (expected 'id' to be str, was {})".format(type(raw["id"])))
                self.id = raw["id"]

        def __init__(self, raw):
            if type(raw) is not dict:
                raise ValueError("Invalid players object (expected dict, found {}".format(type(raw)))

            if "online" not in raw:
                raise ValueError("Invalid players object (no 'online' value)")
            if type(raw["online"]) is not int:
                raise ValueError(
                    "Invalid players object (expected 'online' to be int, was {})".format(type(raw["online"])))
            self.online = raw["online"]

            if "max" not in raw:
                raise ValueError("Invalid players object (no 'max' value)")
            if type(raw["max"]) is not int:
                raise ValueError("Invalid players object (expected 'max' to be int, was {})".format(type(raw["max"])))
            self.max = raw["max"]

            if "sample" in raw:
                if type(raw["sample"]) is not list:
                    raise ValueError(
                        "Invalid players object (expected 'sample' to be list, was {})".format(type(raw["max"])))
    ...
    class Version:
        def __init__(self, raw):
            if type(raw) is not dict:
                raise ValueError("Invalid version object (expected dict, found {}".format(type(raw)))

            if "name" not in raw:
                raise ValueError("Invalid version object (no 'name' value)")
            if not isinstance(raw["name"], str):
                raise ValueError(
                    "Invalid version object (expected 'name' to be str, was {})".format(type(raw["name"])))
            self.name = raw["name"]
            ...
            if type(raw["protocol"]) is not int:
                raise ValueError(
                    "Invalid version object (expected 'protocol' to be int, was {})".format(type(raw["protocol"])))
